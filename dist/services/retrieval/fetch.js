...
const DEFAULT_HEADERS = {
    "user-agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124 Safari/537.36 complementary-health-agent/0.1",
    accept: "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,*/*;q=0.8",
};
export const domainAuthority = (domain) => {
    ...
    if (value.endsWith(".edu"))
        return 0.9;
    if (/(clinic|hospital|healthsystem|medicalcenter|mskcc|mayo|clevelandclinic)/.test(value))
        return 0.88;
    if (/(org)$/.test(value))
        return 0.68;
    return 0.45;
};
export const detectSourceTier = (domain, title = "") => {
...
export const detectEvidenceType = (title, text, tier) => {
    const combined = `${title} ${text.slice(0, 3000)}`.toLowerCase();
    if (combined.includes("meta-analysis"))
        return "meta_analysis";
    if (combined.includes("systematic review"))
        return "systematic_review";
    if (combined.includes("randomized") || combined.includes("randomised") || combined.includes("trial"))
        return "rct";
    if (combined.includes("cohort") || combined.includes("observational"))
        return "observational";
    if (tier === "official")
        return "official_guidance";
    if (tier === "hospital")
        return "expert_patient_education";
    if (tier === "traditional")
...
const buildDocFromHtml = (url, html, hit, retrievalMethod) => {
...
export const fetchDocument = async (hit) => {
    try {
        ...
        if (doc.text.length > 1200 || !config.usePlaywrightFallback)
            return doc;
    }
    catch {
...
export const describeSource = (doc) => `${doc.title} (${doc.domain}, ${doc.sourceTier}, ${doc.evidenceType}) — ${shortText(doc.snippet || doc.text, 160)}`;
