import { ImageLicense, SafeSearchType, search, searchImages, } from "duck-duck-scrape";
import { config } from "../../config.js";
const scoreTierFromDomain = (domain) => {
...
const normalizeWebHit = (query, result) => ({
    query,
    title: result.title,
    url: result.url,
...
export class DuckDuckGoSearchService {
        ...
        for (const query of groups) {
            try {
                ...
                for (const item of result.results.slice(0, config.maxWebResultsPerQuery)) {
                    ...
                    if (!deduped.has(hit.url))
                        deduped.set(hit.url, hit);
                }
            }
            catch {
    ...
    async searchRemedyImages(query) {
        const aggregate = [];
        ...
        for (const offset of [0, 30, 60]) {
            ...
            if (aggregate.length >= config.maxImageCandidatesPerRemedy)
                break;
            await new Promise((resolve) => setTimeout(resolve, 1200));
        }
        const deduped = new Map();
        for (const image of aggregate) {
            if (!deduped.has(image.imageUrl))
                deduped.set(image.imageUrl, image);
        }
