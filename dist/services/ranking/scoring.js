...
export const rankClaims = (claims, topN) => {
    ...
    const remedies = [...grouped.values()].map((bucket) => {
        ...
        const safetyConfidence = clamp(average(bucket.map((claim) => claim.safetyNotes.length ? MODALITY_SAFETY[claim.modality] - 0.08 : MODALITY_SAFETY[claim.modality])));
        ...
        const primaryScore = 0.35 * evidenceQuality +
            0.25 * independentConsensus +
            0.15 * authority +
            0.15 * safetyConfidence +
            0.10 * specificity;
        const secondaryTopMatchScore = 0.55 * normalizedOccurrenceCount +
            0.2 * independentConsensus +
            0.15 * authority +
            0.1 * specificity;
        const evidenceConfidence = primaryScore >= 0.78 ? "high" : primaryScore >= 0.6 ? "moderate" : primaryScore >= 0.42 ? "low" : "insufficient";
        return {
            ...
            instructionSummary: shortText(unique(bucket.map((claim) => claim.instructionSummary).filter(Boolean)).join(" "), 500),
