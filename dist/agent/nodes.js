...
const hasAny = (query, needles) => needles.some((needle) => query.includes(needle));
const detectModalities = (query) => {
    const lowered = query.toLowerCase();
    const explicit = Object.entries(MODALITY_HINTS)
...
const extractKeyTerms = (query) => unique(tokenize(query)).filter((token) => !["best", "most", "effective", "give", "provide", "natural", "top"].includes(token));
...
const heuristicClaimsFromDocument = (query, doc, modalities) => {
    ...
    return REMEDY_LEXICON.filter((item) => modalities.includes(item.modality))
        ...
        const specificity = Math.max(overlapScore(localContext, queryTokens), overlapScore(`${doc.title} ${doc.snippet}`, queryTokens));
        return {
...
const enrichClaimsWithLlm = async (query, doc) => {
    if (!llmAvailable())
        return [];
...
export const initializeNode = async (state) => {
...
export const scopeNode = async (state) => {
...
export const routeAfterScope = (state) => (state.status === "out_of_scope" ? "report" : "plan");
...
export const fetchNode = async (state) => {
...
export const rankNode = async (state) => {
...
const buildImageQueriesForRemedy = (query, remedy) => {
    const suffix = remedy.modality === "acupressure"
...
export const imageNode = async (state) => {
    const withImages = [];
...
export const reportNode = async (state) => {
    const report = {
        ...
        status: (state.status === "out_of_scope" ? "out_of_scope" : "completed"),
        disclaimer: "This agent provides complementary-health information for non-emergency queries. Primary results are evidence-first. The secondary appendix reflects top web matches only and can overstate repeated low-quality content.",
        methodology: [
